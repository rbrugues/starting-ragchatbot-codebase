...
class TestRAGSystem:
        ...
        with patch.object(
            ...
            def mock_processing(file_path):
                if file_path.endswith((".pdf", ".docx", ".txt")):
                    ...
                    chunks = [
                        CourseChunk(
                            course_title=f"Course {course_name}",
                            lesson_number=1,
                            chunk_index=0,
                            content="content",
                        )
        ...
        with patch.object(self.rag_system.vector_store, "clear_all_data") as mock_clear:
            with patch("os.path.exists", return_value=True):
                with patch("os.listdir", return_value=[]):
                    self.rag_system.add_course_folder(
        ...
        with patch("ai_generator.anthropic.Anthropic"):
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        self.rag_system.tool_manager, "reset_sources"
                    ) as mock_reset_sources:
                        mock_get_sources.return_value = [
                            {"text": "Test Source", "link": "http://example.com"}
                        ]
    ...
    def test_get_course_analytics(self):
        ...
        with patch.object(
            self.rag_system.vector_store, "get_course_count"
        ) as mock_count:
            with patch.object(
                self.rag_system.vector_store, "get_existing_course_titles"
            ) as mock_titles:
    ...
    def test_integration_with_real_components(self):
        ...
        test_chunks = [
            ...
            CourseChunk(
                ...
                lesson_number=2,
                chunk_index=1,
                content="Advanced topics discussion",
            ),
        ]
